...
const ChatSupport: React.FC = () => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
          <input value={reply} onChange={e => setReply(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendReply()} placeholder={t('chat.replyPlaceholder')} style={{ flex: 1, padding: '10px 16px', border: '1px solid #e0e0e0', borderRadius: 8, fontSize: 14 }} />
  ...
  return (
    <div>
      <h1 style={{ margin: '0 0 24px', fontSize: 24, fontWeight: 700 }}>{t('chat.title')}</h1>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16, marginBottom: 24 }}>
        <StatCard label={t('chat.openConversations')} value={threads.length} color="#1B3A5C" icon={<MessageSquare size={20} />} />
        <StatCard label={t('chat.totalMessages')} value={threads.reduce((s, th) => s + th.message_count, 0)} color="#43A047" icon={<MessageSquare size={20} />} />
      </div>

      <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        {threads.length === 0 && <div style={{ padding: 40, textAlign: 'center', color: '#999' }}>{t('chat.noConversations')}</div>}
        {threads.map((thread, i) => (
          <div key={i} onClick={() => openThread(thread.user_id, thread)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px', borderBottom: '1px solid #f5f5f5', cursor: 'pointer', transition: 'background 0.15s' }}
            onMouseEnter={e => (e.currentTarget.style.background = '#f8f9fb')}
            onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
            <div style={{ width: 42, height: 42, borderRadius: '50%', background: '#1B3A5C', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 16, fontWeight: 700 }}>{thread.user_name?.[0]}</div>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontWeight: 600, fontSize: 14 }}>{thread.user_name}</span>
                <span style={{ fontSize: 11, color: '#999' }}>{thread.last_at}</span>
              </div>
              <div style={{ fontSize: 12, color: '#666', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 400 }}>{thread.last_message}</div>
            </div>
            <span style={{ background: '#E3F2FD', color: '#1565C0', padding: '2px 8px', borderRadius: 10, fontSize: 11, fontWeight: 600 }}>{thread.message_count}</span>
          </div>
        ))}
      </div>
    </div>
