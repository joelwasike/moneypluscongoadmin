...
const Wallets: React.FC = () => {
  ...
  return (
    <div>
      <h1 style={{ margin: '0 0 24px', fontSize: 24, fontWeight: 700 }}>{t('wallets.title')}</h1>
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <div style={{ position: 'relative', marginBottom: 16, maxWidth: 300 }}>
          ...
          <input placeholder={t('wallets.searchPlaceholder')} value={search} onChange={e => setSearch(e.target.value)} style={{ width: '100%', padding: '8px 12px 8px 36px', border: '1px solid #e0e0e0', borderRadius: 8, fontSize: 14 }} />
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr style={{ borderBottom: '2px solid #f0f0f0' }}>
            <th style={th}>{t('wallets.columns.id')}</th><th style={th}>{t('wallets.columns.userId')}</th><th style={th}>{t('wallets.columns.name')}</th><th style={th}>{t('wallets.columns.type')}</th><th style={th}>{t('wallets.columns.currency')}</th><th style={th}>{t('wallets.columns.balance')}</th><th style={th}>{t('wallets.columns.status')}</th>
          </tr></thead>
          <tbody>
            {wallets.map((w: any) => (
              <tr key={w.id} style={{ borderBottom: '1px solid #f5f5f5' }}>
                ...
                <td style={td}><span style={{ background: w.is_active ? '#E8F5E9' : '#FFEBEE', color: w.is_active ? '#2E7D32' : '#C62828', padding: '2px 8px', borderRadius: 4, fontSize: 12 }}>{w.is_active ? t('common.active') : t('common.inactive')}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
