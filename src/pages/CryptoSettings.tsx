...
export default function CryptoSettings() {
  ...
  const walletStats = [
    { label: t('crypto.totalWallets'), value: '847', icon: <Wallet size={22} color="#fff" />, bg: NAVY },
    { label: t('crypto.activeWallets'), value: '623', icon: <CheckCircle size={22} color="#fff" />, bg: GREEN },
    { label: t('crypto.flagged'), value: '12', icon: <AlertTriangle size={22} color="#fff" />, bg: '#E65100' },
    { label: t('crypto.frozen'), value: '3', icon: <XCircle size={22} color="#fff" />, bg: '#C62828' },
  ];
  ...
  const toggleToken = (idx: number) => {
    setTokens(prev => prev.map((tok, i) => i === idx ? { ...tok, enabled: !tok.enabled } : tok));
  };
  ...
  return (
    <div style={{ fontFamily: FONT, padding: 32, minHeight: '100vh' }}>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, color: NAVY }}>{t('crypto.title')}</h1>
        <p style={{ margin: '4px 0 0', fontSize: 14, color: '#6B7280' }}>{t('crypto.subtitle')}</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
          ...
          <div style={rowStyle}>
            <div>
              <div style={{ fontSize: 14, color: NAVY, fontWeight: 500 }}>{t('crypto.cryptoModuleEnabled')}</div>
              <div style={{ fontSize: 12, color: '#7C8D9E', marginTop: 2 }}>{t('crypto.cryptoModuleEnabledDesc')}</div>
            </div>
            <Toggle on={settings.cryptoEnabled} onToggle={() => update('cryptoEnabled', !settings.cryptoEnabled)} />
          </div>
          <div style={rowStyle}>
            <div>
              <div style={{ fontSize: 14, color: NAVY, fontWeight: 500 }}>{t('crypto.autoApproveWithdrawals')}</div>
              <div style={{ fontSize: 12, color: '#7C8D9E', marginTop: 2 }}>{t('crypto.autoApproveWithdrawalsDesc')}</div>
            </div>
            <Toggle on={settings.autoApproveWithdrawals} onToggle={() => update('autoApproveWithdrawals', !settings.autoApproveWithdrawals)} />
          </div>
          <div style={rowStyle}>
            <div>
              <div style={{ fontSize: 14, color: NAVY, fontWeight: 500 }}>{t('crypto.kycRequired')}</div>
              <div style={{ fontSize: 12, color: '#7C8D9E', marginTop: 2 }}>{t('crypto.kycRequiredDesc')}</div>
            </div>
            <Toggle on={settings.requireKycForCrypto} onToggle={() => update('requireKycForCrypto', !settings.requireKycForCrypto)} />
          </div>
          <div style={rowStyle}>
            <div>
              <div style={{ fontSize: 14, color: NAVY, fontWeight: 500 }}>{t('crypto.maxDailyWithdrawal')}</div>
              <div style={{ fontSize: 12, color: '#7C8D9E', marginTop: 2 }}>{t('crypto.maxDailyWithdrawalDesc')}</div>
            </div>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
          ...
          <div style={rowStyle}>
            <div>
              <div style={{ fontSize: 14, color: NAVY, fontWeight: 500 }}>{t('crypto.confirmationsRequired')}</div>
              <div style={{ fontSize: 12, color: '#7C8D9E', marginTop: 2 }}>{t('crypto.confirmationsRequiredDesc')}</div>
            </div>
            <input style={inputStyle} value={settings.confirmationsRequired} onChange={e => update('confirmationsRequired', e.target.value)} />
          </div>
          <div style={rowStyle}>
            <div>
              <div style={{ fontSize: 14, color: NAVY, fontWeight: 500 }}>{t('crypto.antiFraud')}</div>
              <div style={{ fontSize: 12, color: '#7C8D9E', marginTop: 2 }}>{t('crypto.antiFraudDesc')}</div>
            </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead>
                <tr style={{ backgroundColor: '#F9FAFB', borderBottom: '1px solid #E5E7EB' }}>
                  {[
                    t('crypto.columns.symbol'),
                    t('crypto.columns.name'),
                    t('crypto.columns.minWithdraw'),
                    t('crypto.columns.maxWithdraw'),
                    t('crypto.columns.fee'),
                    t('crypto.columns.enabled'),
                  ].map(h => (
                    <th key={h} style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 28 }}>
        <button onClick={() => showToast(t('crypto.saveSuccess'))} style={{
